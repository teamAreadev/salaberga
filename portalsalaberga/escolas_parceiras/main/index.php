...
<header x-data="{ mobileMenuOpen: false, mobileDropdownOpen: false, init() { this.$watch('mobileMenuOpen', value => { if (value) { document.body.classList.add('overflow-hidden'); } else { document.body.classList.remove('overflow-hidden'); } }); window.addEventListener('resize', () => { if (window.innerWidth >= 1024) { this.mobileMenuOpen = false; } }); } }" class="bg-ceara-green text-ceara-white sticky top-0 z-50 shadow-md">
    ...
    <button @click="isOpen = !isOpen" 
            ...
            type="button"
            class="flex items-center text-ceara-white hover:text-ceara-orange transition duration-300 header-link focus:outline-none">
    ...
    <div x-show="isOpen"
         ...
         class="absolute right-0 w-48 bg-white mt-2 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-50"
         style="display: none;">
        <div class="py-1">
            <a href="../main/views/autenticação/precadastro.php"    
               @click="isOpen = false"
               class="block px-4 py-3 text-sm text-gray-700 hover:bg-ceara-orange hover:text-white transition-all duration-200 ease-in-out flex items-center space-x-2 border-b border-gray-100 group">
                <i class="fas fa-school text-ceara-orange group-hover:text-white transition-colors duration-200"></i>
                <span class="transform group-hover:translate-x-1 transition-transform duration-200">Salaberga</span>
            </a>
            <a href="../view/parceiros/parceiros.html" 
               @click="isOpen = false"
               class="block px-4 py-3 text-sm text-gray-700 hover:bg-ceara-orange hover:text-white transition-all duration-200 ease-in-out flex items-center space-x-2 group">
                <i class="fas fa-handshake text-ceara-orange group-hover:text-white transition-colors duration-200"></i>
                <span class="transform group-hover:translate-x-1 transition-transform duration-200">Escolas Parceiras</span>
            </a>
        </div>
    </div>
    ...
    <button @click="isOpen = !isOpen" 
            type="button"
            class="flex items-center w-full text-ceara-white hover:text-ceara-orange transition duration-300 header-link">
        <i class="fas fa-school mr-1"></i>
        <span>Escolas</span>
        <i class="fas fa-chevron-down ml-1 text-xs transition-transform duration-200"
           :class="{ 'transform rotate-180': isOpen }"></i>
    ...
    <div x-show="isOpen"
        ...
        <a href="../main/views/autenticação/precadastro.php" 
           @click="isOpen = false; mobileMenuOpen = false"
           class="block text-ceara-white hover:text-ceara-orange transition duration-300 flex items-center space-x-2">
            <i class="fas fa-school"></i>
            <span>Salaberga</span>
        </a>
        <a href="#escolas-parceiras" 
           @click="isOpen = false; mobileMenuOpen = false"
           class="block text-ceara-white hover:text-ceara-orange transition duration-300 flex items-center space-x-2">
